...
const __uint32_t DEFAULT_EVENT = EPOLLIN | EPOLLET | EPOLLONESHOT;//读&边沿触发&一次性
  ...
  do {
    ...
    if (state_ == STATE_PARSE_URI) {
      ...
      if (flag == PARSE_URI_AGAIN)
        ...
        perror("2");
        LOG << "FD = " << fd_ << "," << inBuffer_ << "******";//把错误信息记录LOG
        inBuffer_.clear();
        error_ = true;
        handleError(fd_, 400, "Bad Request");
        break;
    ...
    if (state_ == STATE_RECV_BODY) {//接收请求体，就干了一件事，检查请求体是否完整
      ...
      if (static_cast<int>(inBuffer_.size()) < content_length) break;//请求体不完整
      state_ = STATE_ANALYSIS;
    }
...
URIState HttpData::parseURI() {
  ...
  string cop = str;//保存了又不用，保存了个寂寞
  // 读到完整的请求行再开始解析请求
  size_t pos = str.find('\r', nowReadPos_);
  if (pos < 0) {//未找到回车符，结束解析
    return PARSE_URI_AGAIN;
  }
  // 去掉请求行所占的空间，节省空间
  string request_line = str.substr(0, pos);//截取回车符前面的字符，即首行字符
  if (str.size() > pos + 1)
    str = str.substr(pos + 1);//为什么\n也保留了
  else
    str.clear();//只有请求行，干脆都不要了
  // Method
  ...
  if (pos < 0)
    ...
    if (request_line.size() - pos <= 3)//后面没空格了，所以这样判断
      return PARSE_URI_ERROR;
    else {
...
HeaderState HttpData::parseHeaders() {
  ...
  int now_read_line_begin = 0;//当前解析行的开头下标
  bool notFinish = true;
  size_t i = 0;
  for (; i < str.size() && notFinish; ++i) {
    switch (hState_) {
      case H_START: {
        if (str[i] == '\n' || str[i] == '\r') break;//请求行遗留下来的'\n'被直接跳过
        hState_ = H_KEY;
        key_start = i;
      ...
      case H_KEY: {
        if (str[i] == ':') {//找key的结尾':'
          key_end = i;
          if (key_end - key_start <= 0) return PARSE_HEADER_ERROR;//这种情况可能吗
          hState_ = H_COLON;
        } else if (str[i] == '\n' || str[i] == '\r')//要找的是key的结尾':'
          return PARSE_HEADER_ERROR;
        break;
      }
      case H_COLON: {//当前状态是冒号，找冒号后有没有空格
        if (str[i] == ' ') {
      ...
      case H_VALUE: {//找value的结尾'\r'
        if (str[i] == '\r') {
          hState_ = H_CR;
          value_end = i;
          if (value_end - value_start <= 0) return PARSE_HEADER_ERROR;
        } else if (i - value_start > 255)//value太长
          return PARSE_HEADER_ERROR;
        break;
      }
      case H_CR: {
      ...
      case H_END_LF: {//已经读到表示结束的'\n'，结束了
        notFinish = false;
        key_start = i;//这里应该是请求体的下标了吧
        now_read_line_begin = i;
        break;
      }
    }
  ...
  if (hState_ == H_END_LF) {
    str = str.substr(i);//剩下的就是请求体了，但如果没有请求体，这里i不就越界了？
